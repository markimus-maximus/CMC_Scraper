...
The `__get_soup(url:str)` method uses `requests` and `url` from a list of URLS (Milestone 2) to access the webpage. A beutiful soup object is then generated to extract all of the HTML. A 0.5 second pause was included to prevent skipping webpages when multiple webpages were scraped iteratively. In addition, the __get_crypto_rows method generates a list of the URLS accessed.
2. Extracting data from a row.  
Rows include data from one cryptocurrency. For example all of the data associated with bitcoin on one day is contained within a row. The method `__get_crypto_rows(url_list:list, num_page:int)` uses `soup.find_all('tr', attrs={'class':'cmc-table-row'})` to extract all rows.
